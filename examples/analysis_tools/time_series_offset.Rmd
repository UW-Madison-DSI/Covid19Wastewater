...
totalcases <- Case_data %>% group_by(date) %>% summarise(sum_case = sum(conf_case)) %>% mutate(conf_case = roll_sum(sum_case,10,align = "right", fill = NA))
...
covarnames <- c("Robin1","Alpha.V1","Delta","Omicron.21K","Omicron.21L","Omicron.22C","Omicron.22B")
