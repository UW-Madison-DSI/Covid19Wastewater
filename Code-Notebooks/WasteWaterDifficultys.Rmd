...
HFGCaseDFroll2%>%
  ...
  scale_x_date(limits=c(mdy("1/10/2021"),mdy("4/1/2021")))+
...
HFGFrame%>%
  ...
  group_by(Site)%>%
  summarize(leng=max(Date)-min(Date),NDays=length(unique(Date)))
...
ggarrange(p1,p2,nrow = 2,align="hv",common.legend = T)
```
...
ggarrange(plotlist = list(p1,p2),nrow = 2,align="hv",common.legend = T)
