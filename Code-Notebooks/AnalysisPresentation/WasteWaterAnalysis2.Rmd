...
LoessSmoothPlot <- function(SiteS="Madison",
  ...
  LimitLIMSDF=Data%>%
    ...
    LimitLIMSDF$Pred1 <- LoessGenerater(LimitLIMSDF,weights=weights[[1]],
    Span=Span,min = min,max = max)
    LimitLIMSDF$PredSE <- LoessGenerater(LimitLIMSDF,weights=weights[[2]],
    Span=Span,min = min,max = max)
...
```{r HFG variance Fin,fig.height = 6,fig.width = 12,echo=FALSE}
N1PlotGraphic+
  theme(axis.text.x = element_text(size = ConfigOption$XAxisLabSiz, colour = "black"),
     axis.title.x = element_text(size = ConfigOption$GenFontSiz, colour = "black"),
     plot.margin = unit(c(.5,0,0,0), "cm"))+
...
Due to the large daily variation of the N1 concentration we have tried to create a smoothed version of the data set that captures the N1 concentration. Below is our best attempt at smoothing. N2 shows a similar patern.
...
To use standard error in our smoothing we need an error estimate independent of the concentration. 
SE/N1 is an effective weighting factor.
...
Analysis of loess residuals suggest in progress data cleaning
...
# 
# 
# This method is extended for the interceptors. Data from before January is missing due to it not being in the LIMS system. older version of that data is not used because it is missing the standard error for N1 and includes data that has been reran
...
```{r, echo=FALSE,include=FALSE,fig.height=8,fig.width=14}
...
This weighted smoothing is also applied to the UW data
