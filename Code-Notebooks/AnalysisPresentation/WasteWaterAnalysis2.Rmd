...
LoessSmoothPlot <- function(SiteS,Data=LIMSFullDF,Span=.3,min=1e-5,max=1e5,BoxWidth=.5,Independent="N1",Dependent="N1Error",HasNo=F, Title=T,FullLim=F,SiteLab=T,NoLabel=F){
  ...
  DFSize=dim(LIMSDF)[1]
  ConWeightModel=loessFit(y=log(LIMSDF$Indy),
                       x=LIMSDF$Date,
  ...
  SEWeightModel=loessFit(y=log(LIMSDF$Indy),
                       x=LIMSDF$Date,
                       weights=LIMSDF$Indy/LIMSDF$Dep,
                       span=Span,min.weight=min,
                       max.weight=max,
                       iterations=10)
  LIMSDF$Pred1 <- exp(ConWeightModel$fitted)
  LIMSDF$PredSE <- exp(SEWeightModel$fitted)
  LoessPlot=LIMSDF%>%
...
CasePlot=CasePlotMaking("Madison")+xlab("")
