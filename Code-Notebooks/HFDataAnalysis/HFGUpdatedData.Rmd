...
LIMSMadDF <- LIMSFullDF%>%
  filter(Site == "Madison")%>%
  select(-PMMoV,-BCoV,-FlowRate,-Pop,-AVG,-wt)
...
#CCF to find best measurement
#detrended
#if we have full analysis done we  can compare the results of the three
#know N1 N2 are highly correlated (and so is AVG) so anything is ok
#outliers
#
```
...
test <- HFGFrameUp%>%
  group_by(Site,Date)%>%
  mutate(seN1=sqrt(var(N1)/n()),seN2=sqrt(var(N2)/n()),seAVG=sqrt(var(AVG)/n()))%>%
  mutate(a=N1/seN1,b=N2/seN2,c=AVG/seAVG)

Alp=.1
test%>%
  ggplot()+
  geom_point(aes(x=N1,y=seN1,color="N1"),alpha=Alp)+
  geom_point(aes(x=N2,y=seN2,color="N2"),alpha=Alp)+
  geom_point(aes(x=AVG,y=seAVG,color="AVG"),alpha=Alp)+
  scale_y_log10()+
  scale_x_log10()+
  facet_wrap(~Site)

  
t.test(test$c,test$a)
...
#mean of filters
#9 well rep are not iid
#3 wells are assumed iid
#Brian anova 

#To do HFG
#look at covarience
#high population figures
#6) look at other covarients 
#5) Reproduce old work in HFG
#4)  N1 Vs N2 vs avg var
#3)  PMMoV %BCoV
#2) look overall to see if its less noisy
#1)  Madison longitudinal data - see if data is cleaner / does arithmetic mean agree with LIMs N1 / see if SE agrees
#ask for all 10 treatment plants
#?) look at case data if it looks great
#?) water treatment at other stations
