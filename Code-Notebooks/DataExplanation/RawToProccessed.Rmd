...
LIMSDFParsedFN <- paste0(BaseDir,"COVID-19_WastewaterAnalysis/data/processed/LIMSWasteData_2021-06-30_17-40.csv")
...
#write.csv(MadisonCaseDF,paste0(BaseDir,"COVID-19_WastewaterAnalysis/data/processed/MMSD_Cases_processed.csv"),row.names=FALSE)
...
full_join(A,B,by=c("Site","Date","Filter","Well"))%>%
  filter(abs(N1.x-N1.y)<100)
#11 within 100

MergedDF <- full_join(A,B,by=c("Site","Date","Filter","Well"))%>%
  mutate(DIFF = N1.x-N1.y,
         Filter = as.factor(Filter))

DiffPlot <- MergedDF%>%
  ggplot()+
  aes(x=Date,y=DIFF,color = Filter)+
  geom_point(size=.25)+
  facet_wrap(~Site,scales="free")
