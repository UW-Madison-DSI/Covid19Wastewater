...
library(readxl)
BaseDir <- "../../Data/"
HFGCases2FN  <-  paste0(BaseDir,"Source/HFG_data_corrected_w_HF183&CrP_011422.xlsx")
...
viewFunc <- function(Vec){
  return(unique(ifelse(is.na(as.numeric(Vec)),Vec,NA)))
}

HFGWaste%>%
  select(Filter,Well,N1Ct,N1,N2CT,N2,PMMOVCT,PMMOV,BCoV,HF183CT,HF183,CrPCT,CrP)%>%
  as.list()%>%
  lapply(viewFunc)%>%
  unlist()%>%
  unique()

HFGWasteSave <- HFGWaste%>%
  filter(!is.na(Site))%>%
  mutate(Date = as.Date(ifelse(is.na(as.integer(Date)), 
                                     mdy(Date),
                       as.Date(as.integer(Date),
                             origin = "1899-12-30")),
                origin = "1970-01-01"),
        across(c(Filter,Well,N1Ct,N1,N2CT,N2,PMMOVCT,PMMOV,BCoV,HF183CT,HF183,CrPCT,CrP), 
               ~ ifelse(is.na(as.numeric(.x)),NA,.x)))#%>%


write.csv(HFGWasteSave,paste0(BaseDir,"Processed/HFGWasteData011422.csv"),row.names=FALSE)

HFGWasteDF <- read.csv(paste0(BaseDir,"Processed/HFGWasteData011422.csv"))%>%
  mutate(Date=as.Date(Date))
