...
server <- function(input, output) {
  ...
  output$loessPlot <- renderPlotly({
    ...
    LoessGraphic=ModelCDF()%>%
      ...
      geom_rect(aes(ymin=ErrorMin,ymax=ErrorMax,xmin=Date-BoxWidth,xmax=Date+BoxWidth,fill=CapedValue),alpha=.5)+
      geom_rect(aes(ymin=N1,ymax=N1,xmin=Date-BoxWidth,xmax=Date+BoxWidth),alpha=.5)+
      #,size=PointSize
