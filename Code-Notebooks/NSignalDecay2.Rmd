  ...
  GraphPlot <- DF%>%
    ...
    geom_label(inherit.aes=FALSE, 
          aes(x = as.Date(Locate[1]-Locate[2]*abs(as.numeric(as.factor(WWTP))-2)),
           y = Locate[3]+Locate[4]*2*(as.numeric(as.factor(Filter.Replicate))-1.5),
           color=Filter.Replicate, 
...
```{r ploting N2, include=TRUE, warning=FALSE, message=FALSE,fig.height=6}
...
```{r ploting PMMoV, include=TRUE, warning=FALSE, message=FALSE,fig.height=6}
...
```{r ploting BCoV, include=TRUE, warning=FALSE, message=FALSE,fig.height=6}
...
knitr::kable(signif(rbind(`N1 using six replicates`,
