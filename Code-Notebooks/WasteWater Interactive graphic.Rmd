...
server <- function(input, output) {
  ...
  MinDate=reactive({min(LMFData()$Date, na.rm=T)})
  MaxDate=reactive({max(LMFData()$Date, na.rm=T)})
  output$plot2<-renderPlot({
    ggplot(data=LMFData())+geom_point(aes(x=Date,y=roll))+
      geom_point(aes(x=Date,y=Fitted),color="red")+
      ylab("% of test positive")+
      annotate("label", x = max(MaxDate())-10, y = .001, label = paste("R^2:",round(summary(LM())$r.squared,3)),color="red")+
