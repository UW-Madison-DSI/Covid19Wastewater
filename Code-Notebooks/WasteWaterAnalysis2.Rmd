...
# 
# full_join(LatCaseDFRoll,LatCaseDF,by=c("Date","Site"),suffix=c(".Roll",".Norm"))%>%
#   select(Date,Site,Cases.Roll,Cases.Norm)%>%
#   mutate(rollingTest=(
#   lag(Cases.Norm,n=1)+
#   lag(Cases.Norm,n=2)+
#   lag(Cases.Norm,n=3)+
...
annotate_figure(ggarrange(CasePlot,N1Plot,nrow=2,common.legend = TRUE ,align ="hv",legend="bottom"),top = text_grob("Weighted loess smoothing compared to cases)", color = "black", face = "bold", size = 14))
annotate_figure(ggarrange(PerPosPlot,N1Plot,nrow=2,common.legend = TRUE,align ="hv",legend="bottom"),top = text_grob("Weighted loess smoothing compared to percent positive)", color = "black", face = "bold", size = 14))
...
To DO: Redo Labels for DHS audience?
...
```{r, echo=FALSE,include=TRUE, fig.width=15,fig.height=18}
...
LIMS data given does not include all the intercepter data. Older data given does not have N1 Error.


```{r Missing LIMS, warning=FALSE, fig.width=15,fig.height=18,echo=FALSE}
AllDataDF <- full_join(LIMSFullDF,LatWasteDF,by=c("Date","Site"))%>%
  mutate(N1=ifelse(is.na(N1.x),N1.y,N1.x),N1Error=ifelse(is.na(N1Error),1,N1Error))%>%
  select(Date,Site,N1,N1Error)%>%
  filter(Date<mdy("7/1/2021"))




MapList=lapply(c("Madison","MMSD-P2","MMSD-P7","MMSD-P8","MMSD-P11","MMSD-P18"),LoessSmoothPlot,Data=AllDataDF,Span=.3,max=20, Title=F,FullLim=T,NoLabel=T)
#LoessSmoothPlot(SiteS="UW-Sellery",Span=.3,max=20)
#LoessSmoothPlot(SiteS="UW-LakeShore",Span=.3,max=20)
...
Interceptors next to cases

```{r, warning=FALSE, fig.width=9,fig.height=12,echo=FALSE}
N1CaseComp <- function(SiteS){
  N1Plot=LoessSmoothPlot(SiteS=SiteS,Data=AllDataDF,Span=.3,max=20, Title=F,FullLim=F,NoLabel=T)+
  theme()
  CasePlot=CasePlotMaking(SiteS,Method="Per_pos")
