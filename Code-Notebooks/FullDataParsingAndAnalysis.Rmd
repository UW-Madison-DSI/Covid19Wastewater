...
LIMSFullDF%>%
  ggplot()+aes(x=N1,y=N1Error/N1)+geom_point()+
  scale_y_log10()+
  scale_x_log10()+ 
...
VarN1LIMSMode=lm(log(N1Error)~N1-1,data=LIMSFullDF)
...
LIMSWExpErrorDF%>%
  ggplot()+aes(x=N1)+geom_point(aes(y=(N1Error-0.08185*PredN1Var)/(0.08185*PredN1Var)))+
  #geom_point(aes(y=PredN1Var/PredN1Var),color="red")+
  #scale_y_log10()+
  scale_x_log10()+ 
  Header_theme(ConfigOption)+
...
#add 2 week rolling average to confirm concept
#how to scale data to relate
#add formal test
#think about things
#give dates of the case outliers
ui <- fluidPage(
