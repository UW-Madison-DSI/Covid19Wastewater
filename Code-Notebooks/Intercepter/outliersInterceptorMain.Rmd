...
IntercepterChangeDF <- IntercepterDF%>%
  filter(!is.na(N1))%>%
  mutate(ChangeN1 = N1-lag(N1),
         ChangeN2 = N2-lag(N2),
         PerChangeN1 = 200*(N1-lag(N1))/(N1+lag(N1)),
         PerChangeN2 = 200*(N2-lag(N2))/(N2+lag(N2)))


IntercepterChangeOverLay <- IntercepterChangeDF%>%
  ggplot(aes(x=Date))+
  geom_point(aes(y=abs(ChangeN1),color = Site),size = SizeUsed,
             alpha= alphaUsed,shape=N1ShapeUnit)+
  geom_point(aes(y=abs(ChangeN2),color = Site),size = SizeUsed,
...
IntercepterPerChangeOverLay <- IntercepterChangeDF%>%
  ggplot(aes(x=Date))+
  geom_point(aes(y=PerChangeN1,color = Site),size = SizeUsed, alpha= alphaUsed,shape=N1ShapeUnit)+
  geom_point(aes(y=PerChangeN2,color = Site),size = SizeUsed, alpha= alphaUsed,shape=N2ShapeUnit)+
  theme_light()#+
  #scale_y_log10()
...
LoessPlotFunc <- function(SiteFilter,DF,SpanConstant = .163){
  MainDF <- DF%>%
...
SiteLoessDF <- lapply(c("MMSD-P11","MMSD-P18","MMSD-P2","MMSD-P7","MMSD-P8"),
                      LoessPlotFunc,IntercepterDF,SpanConstant=.2)%>%
                bind_rows()
...
```{r Loess smoothing plot}
LoessPlotFunc <- function(SiteFilter,DF){
  MainDF <- DF%>%
    filter(Site==SiteFilter)
