...
MassBalencePlot <- IntercepterDFPure%>%
  ggplot()+
  geom_col(aes(x=Date,y=SC2_mass, fill=Site), position="stack", stat="identity", width = 3) + 
  scale_fill_brewer(palette="Spectral") +
  scale_color_manual(values=c("#d60000", "#ffa600", "#7ed05c", "#5c9c43", "#3a56d6")) +
  theme_light() +
  geom_point(data = MadDF, aes(x=Date,y=SC2_mass),size=1)+
...
BaseGridPlot <- BaseColDF%>%
            ...
            ggplot(aes(x=Date))+
            geom_line(aes(y=loessN1, color = Site, linetype  = "N1"))+
            geom_line(aes(y=loessN2, color = Site, linetype  = "N2"))+
            scale_colour_manual(values = HueSpace)+
            theme_light() +
...
MadChangeOverLay <- MadChangeDF%>%
  ggplot(aes(x=Date))+
  geom_jitter(aes(y = ChangeN1, color = FlagedOutliersN1, shape = "N1"), size = SizeUsed,
             alpha = alphaUsed, stroke = strokeSize,
             width = WidthJit ,height = HeightJit )+
  geom_jitter(aes(y = ChangeN2,color = FlagedOutliersN2, shape = "N2"), size = SizeUsed,
             alpha = alphaUsed, stroke = strokeSize,
             width = WidthJit ,height = HeightJit )+
  scale_shape_manual(values = c(N1ShapeUnit,N2ShapeUnit))+
  scale_colour_manual(values = HueSpace)+
