...
OutLierPlotDF <- MadDF%>%
  ...
  geom_jitter(aes(y=N1, color="inlier", info = N1, shape= "N1"), 
             data=NonOuliersDF, size = .5, stroke = 1,
             width = WidthJit ,height = HeightJit , size = 1.5)+
  geom_jitter(aes(y=N2, color="inlier",  info = N2 , size = 1.5, shape= "N2"), 
             data=NonOuliersDF, size = .5, stroke = 1,
             width = WidthJit ,height = HeightJit )+
  geom_jitter(aes(y=OutlierN1, color="Outlier", info = OutlierN1, shape= "N1"),
             width = WidthJit ,height = HeightJit, stroke = 1 )+
  geom_jitter(aes(y=OutlierN2, color="Outlier", info = OutlierN2, shape= "N2"),
             width = WidthJit ,height = HeightJit, stroke = 1 )+
  theme_light() +
  #scale_y_log10()+
...
IntercepterOverLay <- IntercepterDF%>%
  ...
  ggplot(aes(x=Date))+
  geom_jitter(aes(y=N1,color = Site, shape="N1"),size = SizeUsed, alpha= alphaUsed,
             stroke = strokeSize, width = WidthJit ,height = HeightJit )+
  geom_jitter(aes(y=N2,color = Site, shape="N2"),size = SizeUsed, 
              alpha= alphaUsed, stroke = strokeSize, width = WidthJit ,height = HeightJit )+
  theme_light()+
  scale_shape_manual(values = c(N1ShapeUnit,N2ShapeUnit))+
...
IntercepterChangeOverLay <- IntercepterChangeDF%>%
  ggplot(aes(x=Date))+
  geom_jitter(aes(y = ChangeN1, color = Site, shape = "N1"), size = SizeUsed,
             alpha = alphaUsed, stroke = strokeSize,
             width = WidthJit ,height = HeightJit )+
  geom_jitter(aes(y = ChangeN2,color = Site, shape = "N2"), size = SizeUsed,
             alpha = alphaUsed, stroke = strokeSize,
             width = WidthJit ,height = HeightJit )+
  scale_shape_manual(values = c(N1ShapeUnit,N2ShapeUnit))+
  scale_colour_manual(values = HueSpace)+
