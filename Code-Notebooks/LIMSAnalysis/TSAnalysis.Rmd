...
MadCasedat<-RollPerPos(LatCaseDF,"Cases","Tests",Facet="Site",n=RAN)%>%
  filter(Site=="Madison")
...
MadLIMsdata2%>%
  ggplot()+
  geom_point(aes(x=Date,y=N1UnSmoothed))+
  geom_line(aes(x=Date,y=N1),color="red")+
  scale_y_log10()
```
...
ts.plot(CasesTSVec,
 ...
 legend("topright", legend=c(paste(RAN,"MA Cases"),"Loess Smoothing",paste(RAN, "MA N1")), col=c(2,4,3), lty=1, cex=1)
```
...
alldata1 <- ts.intersect(CasesTSVec,loessPred,
                         N1TSVec,
...
tryit1a  <- lm(CasesTSVec~N1A+N1B+N1C, data = alldata1)
...
#gamma - read and extract gamma dist from emailed paper
#add to shedding lag files - see if it fixes issues in TS
#
```
...
alldata1 <- ts.intersect(CasesTSVec,
                         N1TSVec,
                         N1A=stats::lag(N1TSVec,-20),
                         N1B=stats::lag(N1TSVec,20),
                         N1C=stats::lag(N1TSVec,10),
                         N1D=stats::lag(log(N1TSVec),-20))


tryit1b  <- lm(CasesTSVec~N1A+N1B, data = alldata1)
