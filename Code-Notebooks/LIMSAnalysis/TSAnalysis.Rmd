...
LIMSFullDFAllCols <- LIMSDataPARSER(LIMSFN)
...
WeekdaySmoothedDF <- RollAvg(LIMSFullDF,var=c("N1"))%>%
  mutate(weekday=weekdays(Date))%>%
  filter(N1>0,N1Error>0)
...
MadCasedat<-DFSmoothingFNC(LatCaseDF)%>%
  filter(Site=="Madison")
...
```{r}
#CCF visual relationship
#CDC "Trajectory" first derivative / second derivative
#first wave fits bests - after that it starts to have issues due to SLD
#21 days truncation
 
#can we apply a reverse process to get a leading indicator of cases
#behavior of N1 with changing SLD - might be about derivatives
#might be able to subtract terms to get a view of future cases
 ...
 #14MACASES~14MAN1
#adf.test(CasesTSVec)
#plot(log(tsclean(N1TSVec)))

library(MTS)

data <- cbind(CasesTSVec,log(N1TSVec))
Eccm(data)
mod <- VARMA(data, p = 1, q = 0)
...
alldata1 <- ts.intersect(rollingCase,loessPred,
                         N1TSVec,
                         N1A=stats::lag(loessPred,20),
                         N1B=stats::lag(loessPred,7),
                         N1C=stats::lag(loessPred,-20))


tryit1a  <- lm(rollingCase~N1A+N1B, data = alldata1)
