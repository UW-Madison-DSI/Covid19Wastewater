...
library(limma)
DataPrep <- function(Vec1,Vec,SiteS=NA){
  if(!is.na(SiteS)){
    FilteredVec1 <- filter(Vec1,Site==SiteS)
  }else{
    FilteredVec1 <- Vec1
  }
  ...
  ReadyDF <- full_join(FullDayDF,FilteredVec1, by = c("Date"))%>%
    fill(!!sym(Vec), .direction = "down")%>%
    mutate(Site=SiteS)%>%
    select(Date,!!sym(Vec),Site)
  return(ReadyDF)
}
...
DFLoessFNC <- function(N1DF,SiteS){
    ...
    ReadyDF <- DataPrep(N1DF,"N1",SiteS=SiteS)
...
SLDGraphics <- function(SiteS,DepTSVec,IndTSVec,DepName,IndName,efficient=FALSE){
  TSPloting(list(DepTSVec,IndTSVec),MergedDF,
            FullPlot=FALSE,SubTitle=SiteS)
  TSPloting(list(diff(DepTSVec),diff(IndTSVec)),MergedDF,
            FullPlot=FALSE,SubTitle=SiteS)
  ...
  #           FullPlot=FALSE,SubTitle=SiteS)
  ccf(IndTSVec,DepTSVec,
               main=paste("CC between",IndName,"and", DepName),
               sub=SiteS)
  preWhiteFit <- auto.arima(IndTSVec, seasonal=FALSE,
  stepwise=FALSE, approximation=FALSE)

  x <- IndTSVec - fitted(Arima(IndTSVec, model = preWhiteFit))
  y <- DepTSVec - fitted(Arima(DepTSVec, model = preWhiteFit))
  CCFVec=ccf(x, y,lag.max=22,
               main=paste("prewhiten CC between",IndName,"and", DepName),
               sub=SiteS)
  
  if(max(CCFVec[[1]])<.15&&efficient){
    print("no signifigent relationship")
    return(NULL)
  }
...
SLDModelMad <- SLDGraphics("Madison",SLDCaseVec,logN1Vec,"SLD","Loess")
```
...
```{r Regression with ARIMA errors}
SLDTS <- SLDModel[,6]
N1 <- log(SLDModel[,8])
LoessN1 <- log(SLDModel[,15])
XReg <- do.call(ts.intersect,lapply(-10:10,stats::lag,x=LoessN1))
...
OLM <- lm(SLDTS~stats::lag(LoessN1,11))
FM <- cochrane.orcutt(OLM,max.iter=1000)
summary(OLM)
OLMR <- residuals(OLM)
OLMP <- predict(OLM)
ggtsdisplay(OLMR) #Clear AR relationship
#ggtsdisplay
#checkresiduals
AARIMA <- auto.arima(OLMR, seasonal=FALSE,
  stepwise=FALSE, approximation=FALSE)
ggtsdisplay(residuals(AARIMA))
...
```{R Signal Decoder}
LoessN1Moved=stats::lag(LoessN1,k=14)
ccf(LoessN1Moved,SLDTS)
DoubleData=ts.intersect(LoessN1Moved,SLDTS)
```
