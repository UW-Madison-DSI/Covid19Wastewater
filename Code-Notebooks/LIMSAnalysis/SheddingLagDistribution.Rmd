...
TSPloting <- function(PlotingTS,SourceDF,DepName,IndName,FullPlot=TRUE,SubTitle=NA,FirstDif=FALSE){
  ...
  if(FullPlot){
    ...
    axis(side = 4, at = signif(exp(pretty(range(PlotingTS[[2]]))),1))
  ...
  axis(side = 2, at = pretty(range(PlotingTS[[1]])))
...
TSPloting(list(SLDCaseVec,loessN1Vec,CaseVec,logN1Vec),MergedDF,"A","IndName",
            FullPlot=TRUE,SubTitle="Madison")

signif(exp(pretty(range(logN1Vec))),1)
```
