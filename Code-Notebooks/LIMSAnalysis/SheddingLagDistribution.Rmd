...
MergedDF <- MergedDF%>%
  select(Date,Cases,Cases2,Cases2.PreRolled,N1)
...
MadData <- MergedDF%>%
  mutate(MovedCases = lag(Cases2,2))%>%
...
MadData%>%
  ...
  annotate("text", x=300, y=4e4, label= paste("R^2:", signif(summary(lm(CasesM~N1M,data=MadData))[[8]],3)))+
  annotate("text", x=300, y=1e5, label= paste("Cor:", signif(cor(A,B,use="pairwise.complete.obs"),3)))




# MadData$Resid <- c(NA,resid(lm(CasesM~N1M,data=MadData)))
# MadData%>%
#   ggplot()+
#   geom_line(aes(x=CasesM,y=Resid))

MergedDF%>%
  ggplot()+
  aes(x=Cases2.PreRolled,y=lead(LoessN1,14))+
  geom_point()+
  labs(x="SLD Cases",
       y="loess smoothing of N1")+
  annotate("text", x=300, y=4e4, label= paste("R^2:", signif(summary(lm(Cases2.PreRolled~lead(LoessN1,14),data=MergedDF))[[8]],3)))+
  annotate("text", x=300, y=1e5, label= paste("Cor:", signif(cor(MergedDF$Cases2.PreRolled,lead(MergedDF$LoessN1,14),use="pairwise.complete.obs"),3)))


MergedDF%>%
  ggplot()+
  aes(x=Cases2.PreRolled,y=lead(N1Filtered,LeadN))+
  geom_point()+
  labs(x="SLD Cases")+
  annotate("text", x=300, y=4e4, label= paste("R^2:", signif(summary(lm(log(Cases2.PreRolled)~log(lead(N1Filtered,LeadN)),data=MergedDF))[[8]],3)))+
  annotate("text", x=300, y=1e5, label= paste("Cor:", signif(cor(MergedDF$Cases2.PreRolled,lead(MergedDF$N1Filtered,LeadN),use="pairwise.complete.obs"),3)))
...
# model
# 
# Model1 <- VAR(cbind(SLDPreRolledVec,exp(loessN1Vec)), p = 2, type = "const", season = NULL) 
# summary(Model1)
# 
...
# #Difference
# #
# plot((SLDPreRolledVec),exp(loessN1Vec))
# plot(diff(SLDPreRolledVec),diff(exp(loessN1Vec)))
```
...
# source("../../Scripts/HelperFunctions.R")
# 
# TSPloting2(list(SLDPreRolledVec,exp(loessN1Vec),CaseVec),MergedDF,
