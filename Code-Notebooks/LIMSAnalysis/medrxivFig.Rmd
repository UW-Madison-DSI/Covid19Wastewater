...
TargetSites=c("Madison","Hudson","UW-Sellery","UW-LakeShore")

WasteWaterDF=LIMSFullDF%>%
  filter(Site %in%TargetSites)
...
myplots[[2]]= ggplot(data=WasteWaterDF)+
  aes(x=Date)+
  geom_col(aes(y=BCoV))+
  scale_x_date(limits = c(as.Date("2020-9-4"),as.Date("2021-1-10")),labels = date_format("%b"))+
  theme_bw() +
