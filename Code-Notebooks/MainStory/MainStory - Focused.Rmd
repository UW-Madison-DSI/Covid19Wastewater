...
LIMSFullDF <- ParseData(LIMSFN)%>%
  ...
  select(Date, Site, N1, BCoV , N2 , PMMoV, GeoMeanN12)
...
ErrorMarkedDF <- EarlyDF%>%
  mutate(SmoothVarBase = ifelse(EarlyOutlier,NA,!!sym(SelectedIndVar)),
         ...
         LargeError = !!sym(SelectedIndVar)>(4*SmoothVarMedian)|!!sym(SelectedIndVar)<(SmoothVarMedian/4))
...
ErrorRemovedDF <- ErrorMarkedDF%>%
  mutate(!!OutlierName:=ifelse(EarlyOutlier|LargeError,!!sym(SelectedIndVar),NA),
         ...
         select(-LargeError,-EarlyOutlier)#Removing unneeded calculated columns
