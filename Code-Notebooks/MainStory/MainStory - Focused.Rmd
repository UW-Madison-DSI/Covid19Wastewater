...
IQRRollingFunc <- function(Vector,High=TRUE,Gap=1.5,na.rm=TRUE){
  Base <- quantile(Vector,c(.25,.75),na.rm=na.rm)
  IQR <- IQR(Vector,na.rm=na.rm)
  if(High){
    return(Base[2]+1.5*IQR)
  }else{
   return(Base[1]-1.5*IQR)
  }
}
...
EarlyDF <- FullDF%>%#Remove older Var data that clearly has no relationship to Cases
  mutate(EarlyOutlier = ifelse(Date < mdy("11/20/2020"),TRUE,FALSE))#replace old. 
#"11/20/2020"
#"1/1/2021"
#!!sym(SelectedIndVar)
ErrorMarkedDF <- EarlyDF%>%
  mutate(SmoothVarBase = ifelse(EarlyOutlier,NA,!!sym(SelectedIndVar)),
         SmoothVarMean =rollapply(data = SmoothVarBase, width = DaySmoothed, FUN = mean, 
                            na.rm = TRUE,fill=NA),
         #Fixing issue where rollapply fills NA on right border
         SmoothVarMean = ifelse(is.na(SmoothVarMean),!!sym(SelectedIndVar),SmoothVarMean),
         SmoothVarIQRHigh =rollapply(data = SmoothVarBase, 
                                     width = DaySmoothed,
                                     FUN = IQRRollingFunc, 
                            na.rm = TRUE,fill=NA),
         SmoothVarIQRLow =rollapply(data = SmoothVarBase, 
                                     width = DaySmoothed, 
                                     FUN = IQRRollingFunc, 
                                     High = FALSE,
                            na.rm = TRUE,fill=NA),
         SmoothVarIQRHigh = ifelse(is.na(SmoothVarIQRHigh),!!sym(SelectedIndVar),SmoothVarIQRHigh),
         SmoothVarIQRLow = ifelse(is.na(SmoothVarIQRLow),!!sym(SelectedIndVar),SmoothVarIQRLow),
         SmoothVarMedian =rollapply(data = SmoothVarBase, width = DaySmoothed, FUN = median, 
                            na.rm = TRUE,fill=NA),#Finding very smooth version of the data with no outliers
         SmoothVarMedian = ifelse(is.na(SmoothVarMedian),!!sym(SelectedIndVar),SmoothVarMedian),
         LargeError2 = !!sym(SelectedIndVar)>(4*SmoothVarMedian)|!!sym(SelectedIndVar)<(SmoothVarMedian/4),
         LargeError = !!sym(SelectedIndVar)>SmoothVarIQRHigh|!!sym(SelectedIndVar)<SmoothVarIQRLow)
#Calculating error Limits
                      #was 4
ErrorRemovedDF <- ErrorMarkedDF%>%
  mutate(!!OutlierName := ifelse(EarlyOutlier | LargeError | LargeError2 , !!sym(SelectedIndVar),NA),
         OutlierType = ifelse(LargeError&LargeError2,"Both Detect Outliers" , ifelse(LargeError,"Only IQR","Only Median")),
         
         !!SelectedIndVar := ifelse(EarlyOutlier,NA,!!sym(SelectedIndVar)),
         !!SelectedIndVar := ifelse(LargeError,SmoothVarMedian,!!sym(SelectedIndVar)))%>%#replacing data points that variance is to large
...
#SevenDayMACases
Ref <- ErrorRemovedDF%>%
  NoNa(SelectedIndVar,"Cases")%>%#Removing outliers
  ggplot(aes(x=Date))+#Data depends on time
  geom_line(aes(y=MinMaxFixing(!!sym(SelectedIndVar),Cases),
                color=SelectedIndVar, 
  ...
  geom_line(aes(y=SevenDayMACases, 
                color="Seven Day MA Cases",
                info=Cases))+
  geom_point(aes(y=MinMaxFixing(!!sym(OutlierName),Cases,!!sym(SelectedIndVar)),
                 color=OutlierType,
                 info = !!sym(OutlierName)))+
  geom_line(aes(y=MinMaxFixing(SmoothVarMedian/4,Cases,!!sym(SelectedIndVar)),
                 color="SmoothVarMedian",
                 info = SmoothVarMedian))+
  geom_line(aes(y=MinMaxFixing(4*SmoothVarMedian,Cases,!!sym(SelectedIndVar)),
                 color="SmoothVarMedian",
                 info = SmoothVarMedian))+
  geom_line(aes(y=MinMaxFixing(SmoothVarIQRLow,Cases,!!sym(SelectedIndVar)),
                 color="SmoothVarIQR",
                 info = SmoothVarMedian))+
  geom_line(aes(y=MinMaxFixing(SmoothVarIQRHigh,Cases,!!sym(SelectedIndVar)),
                 color="SmoothVarIQR",
                 info = SmoothVarMedian))+
  labs(y="Reported cases")
  #ColorRule
...
ggplotly(Ref,tooltip=c("info","Date"))%>%
    add_lines(x=~Date, y=ErrorRemovedDF[[SelectedIndVar]],
            yaxis="y2", data=ErrorRemovedDF, showlegend=FALSE, inherit=FALSE) %>%
    layout(yaxis2 = SecondAxisFormat,
           legend=list(title=list(text=''),x = 1.15, y = 0.9),
           margin = list(l = 50, r = 75, b = 50, t = 50, pad = 4))
```
...
RelationShipDF <- SLDSmoothedDF%>%
  select(Date,Cases,SevenDayMACases,SLDCases,N1,N2,BCoV,PMMoV,"N1 Outlier",loessN1,FlowRate,temperature,equiv_sewage_amt)

RelationShipDF%>%
  filter(!is.na(!!sym("N1 Outlier")))
...
ccf(DiffDF$DiffLoessN1,DiffDF$DiffSLDCases,na.action=na.pass)
```
...
ErrorRemoved <- ModelTesting(ErrorRemovedDF,SelectedIndVar,"SevenDayMACases")
