...
OutlierDetectRobustFunc <- function(Vector,method,n = 5,Bin = 21){
...
ErrorRemovedDF <- ErrorMarkedDF%>%
  mutate(!!OutlierName := ifelse(DetectedOutlier, !!sym(SelectedIndVar),NA),
         !!SelectedIndVar := ifelse(EarlyOutlier,NA,!!sym(SelectedIndVar)),
         !!SelectedIndVar := SelectedOutlierMethod)%>%#replacing data points that variance is to large
...
UpdatedDF <- ErrorRemovedDF%>%
  select(-IQRRobustOutlier, -MedianRobustOutlier, -SDMeanRobustOutlier, -SelectedOutlierMethod, -"N1 Outlier"
)
```
