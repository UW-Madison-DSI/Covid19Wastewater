...
knit: (
  function(inputFile, encoding) { 
    ...
    pSubTitle <- paste0('RmdOutput/',Sys.Date(),"_",File,"_Madison");
    rmarkdown::render( 
      input       = inputFile, 
      encoding    = encoding, 
...
LatCaseDF <- MainCaseDataPrep(BaseDir,params$Site)

#Importing the Madison waste water data
LIMSFullDF <- MainWastePrep(BaseDir,params$Site)
...
ErrorMarkedDF <- FullDF%>%
  group_by(Site)%>%
  group_split()%>%
  lapply(TrendSDOutlierFilter, SelectedIndVar, 1.5, 14, n = 5, 
          TrendFunc = LoessSmoothMod, verbose=FALSE)%>%
  bind_rows()%>%
    mutate(NoOutlierVar = ifelse(FlaggedOutlier, NA, !!sym(SelectedIndVar)))

#Split N1 into outlier and non outlier for next ggplot
OutLierPlotDF <- ErrorMarkedDF%>%
  mutate(!!OutlierName := ifelse(FlaggedOutlier,!!sym(SelectedIndVar),NA))%>%
