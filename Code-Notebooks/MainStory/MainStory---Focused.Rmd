...
LIMSFullDF <- ParseData(LIMSWastePath(BaseDir))%>%
  ...
  select(Date, Site, N1 , N2 , PMMoV,
         GeoMeanN12,FlowRate,temperature)

#joining the two data frames together
FullDF <- full_join(LatCaseDF,LIMSFullDF, by = c("Date","Site"))
#library("readxl")

#x <- unique(LIMSFullDF$Site)
#y <- unique(LatCaseDF$Site)
#length(x)
#length(y)
#z <- intersect(x,y)
#length(z)
#x[!(x %in% y)]
#x[grepl("Milwaukee",x)]
#"South Milwaukee WWTF", "Milwaukee MSD Jones Island", "Milwaukee MSD South Shore" 
#y[y=="Milwaukee MSD South Shore"]
```
