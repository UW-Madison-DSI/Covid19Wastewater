...
server <- function(input, output, session) {
  ...
  DF <- reactive({
    ...
    if(0<input$IQR){
      IQRThresh <- OutlierDetectRobustFunc(ErrorMarkedDF$N1, IQRFunc, Gap=input$IQR, Lines = TRUE, Bin = input$DaySmoothed, align = input$Centered, n = input$Robust)
      ErrorMarkedDF$IQRLow <- IQRThresh[[2]]
      ErrorMarkedDF$IQRHigh <- IQRThresh[[3]]
      ErrorMarkedDF <- ErrorMarkedDF%>%
        mutate(DetectedOutlier = ifelse(N1>IQRHigh|N1<IQRLow,TRUE,DetectedOutlier))
    }
    if(0<input$Median){
      MedThresh <- OutlierDetectRobustFunc(ErrorMarkedDF$N1, MedianFunc, Gap = input$Median, Lines = TRUE,Bin=input$DaySmoothed,align=input$Centered, n = input$Robust)
      ErrorMarkedDF$MedLow <- MedThresh[[2]]
      ErrorMarkedDF$MedHigh <- MedThresh[[3]]
      ErrorMarkedDF <- ErrorMarkedDF%>%
        mutate(DetectedOutlier = ifelse(N1>MedHigh|N1<MedLow,TRUE,DetectedOutlier))
    }
    if(0<input$SD){
      SDThresh <- OutlierDetectRobustFunc(ErrorMarkedDF$N1, MeanSDFunc,Gap = input$SD, Lines = TRUE,Bin=input$DaySmoothed,align=input$Centered, n = input$Robust)
      ErrorMarkedDF$SDLow <- SDThresh[[2]]
      ErrorMarkedDF$SDHigh <- SDThresh[[3]]
      ErrorMarkedDF <- ErrorMarkedDF%>%
        mutate(DetectedOutlier = ifelse(N1>SDHigh|N1<SDLow,TRUE,DetectedOutlier))
    }
