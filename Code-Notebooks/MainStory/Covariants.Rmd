...
ErrorMarkedDF <- EarlyDF%>%
  mutate(SmoothVarBase = ifelse(EarlyOutlier,NA,N1),
         ...
         SmoothVarMean = ifelse(is.na(SmoothVarMean),N1,SmoothVarMean),
         ...
         SmoothVarIQRLow =rollapply(data = SmoothVarBase, 
                                     ...
                                     High = FALSE,
                            na.rm = TRUE,fill=NA),
         SmoothVarIQRHigh = ifelse(is.na(SmoothVarIQRHigh),N1,SmoothVarIQRHigh),
         SmoothVarIQRLow = ifelse(is.na(SmoothVarIQRLow),N1,SmoothVarIQRLow),
         ...
         SmoothVarMedian = ifelse(is.na(SmoothVarMedian),N1,SmoothVarMedian),
         LargeError = N1>(4*SmoothVarMedian)|N1<(SmoothVarMedian/4))
```


```{r covariants outliers}

CoVarGraphic <- ErrorMarkedDF%>%
  ggplot()+
  aes(x = log(N1))

CoVarGraphicPMMoV <- CoVarGraphic+
  geom_point(aes(y = PMMoV,color = LargeError))
CoVarGraphicFlowRate <- CoVarGraphic+
  geom_point(aes(y = FlowRate,color = LargeError))
CoVarGraphicBCoV <- CoVarGraphic+
  geom_point(aes(y = BCoV,color = LargeError))
CoVarGraphicequiv_sewage_amt <- CoVarGraphic+#temperature,equiv_sewage_amt
  geom_point(aes(y = equiv_sewage_amt,color = LargeError))
