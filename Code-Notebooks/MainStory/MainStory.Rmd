...
MadisonCaseDF <- read.csv(paste0(BaseDir,"COVID-19_WastewaterAnalysis/data/processed/UpdatedHFGFN.csv"))%>%
...
FullDF3%>%
  NoNa("N1","Cases")%>%#Removing outliers
  ggplot(aes(x=Date))+#Data depends on time
  geom_line(aes(y=MinMaxNormalization(N1), color="N1"))+#compares N1 to Cases
  geom_line(aes(y=MinMaxNormalization(Cases), color="Cases"))+
  labs(y="variable min max normalized")
```
...
SLDSmoothedDF%>%
  ...
  ggplot(aes(x=Date))+
  geom_line(aes(y=MinMaxNormalization(SLDCases), color="SLDCases"))+
  geom_line(aes(y=MinMaxNormalization(N1), color="N1"))+
  facet_wrap(~Site)+
  labs(y="variable min max normalized")
```
...
BinDF%>%
  ...
  ggplot(aes(x=Week))+
  geom_line(aes(y=MinMaxNormalization(BinnedN1), color="N1"))+
  geom_line(aes(y=MinMaxNormalization(BinnedCases), color="Cases"))+
...
SLDSmoothedDF%>%
  ...
  ggplot(aes(x=Date))+
  geom_line(aes(y=MinMaxNormalization(loessN1), color="loessN1"))+
  geom_line(aes(y=MinMaxNormalization(SLDCases), color="SLDCases"))+
  facet_wrap(~Site)+
  labs(y="variable min max normalized")
