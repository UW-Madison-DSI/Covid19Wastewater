...
FullDF2 <- FullDF %>% #Remove older N1 data that clearly has no relationship to Cases
  mutate(N1  =  ifelse(Date < mdy("11/20/2020"),NA,N1))#replace old. make tone more neutral
...
```{r CCF and Granger test 3, include = FALSE}
...
ggplotly(BinnedGraph,tooltip = c("info","Date"))
...
To generate this relationship without reducing the amount of data we rely on a loess smoothing of the data. The loess smoothing [Discription of what loess does and how it optimaly extract trend/continunes ]. The displayed plot shows the visual power of this smoothing. We see a relationship in the big patterns but also multiple sub patterns match. We see in general that smoothed N1 both lags and leads the case data.
