...
CasePlot <- FullDF%>%
  ggplot()+
  geom_point(aes(x=Date,y=FirstConfirmed,color=Site), width = 3) + 
  theme_light() +
  ylab("Cases") +
  ggtitle("Cases, 24h Mass Loading (Madison)")
ggplotly(CasePlot)
...
FlaggedDays <- UsedDF%>%
  ...
  select(Date,Site,Threshold)

IntercepterDF <- full_join(IntercepterDF,FlaggedDays)%>%
  filter(is.na(Threshold))



BasePlot <- IntercepterDF%>%
  filter(!is.na(N1))%>%
  ggplot(aes(x=Date, color = Site))+
  scale_y_log10()


BaseGraphic <- BasePlot+
  geom_point(aes(y=N1),size=.5,alpha=.5)+
  geom_smooth(aes(y=N1),span  = .01, se = FALSE, method = "loess")

FlowUnNormed <- BasePlot+
  geom_point(aes(y=N1*FlowRate),size=.5,alpha=.5)+
  geom_smooth(aes(y=N1*FlowRate),span  = .01, se = FALSE, method = "loess")

FlowPopTrans <- BasePlot+
  geom_point(aes(y=N1*FlowRate/Pop),size=.5,alpha=.5)+
  geom_smooth(aes(y=N1*FlowRate/Pop),span  = .01, se = FALSE, method = "loess")
ggplotly(BaseGraphic)
ggplotly(FlowUnNormed)
ggplotly(FlowPopTrans)
#Pop
#hypothasis everyone has the same covid/person
```
