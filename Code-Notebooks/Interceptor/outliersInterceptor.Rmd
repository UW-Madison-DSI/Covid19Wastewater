...
FlaggedDays <- UsedDF%>%
  dplyr::filter(as.integer(ThresholdOutlier)-1>0)%>%
  select(Date,Site,ThresholdOutlier)
...
BaseGraphic <- BasePlot+
  ...
  geom_smooth(aes(y=N1),span  = .2, se = FALSE, method = "loess")

FlowUnNormed <- BasePlot+
  geom_point(aes(y=N1*FlowRate),size=.5,alpha=.5)+
  geom_smooth(aes(y=N1*FlowRate),span  = .2, se = FALSE, method = "loess")

FlowPopTrans <- BasePlot+
  geom_point(aes(y=N1*FlowRate/Pop),size=.5,alpha=.5)+
  geom_smooth(aes(y=N1*FlowRate/Pop),span  = .2, se = FALSE, method = "loess")
