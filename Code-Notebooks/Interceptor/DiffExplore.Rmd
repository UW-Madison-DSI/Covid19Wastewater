...
DiffGen <- LIMSFullDF%>%
  ...
  group_by(Site)%>%
  arrange(Date)%>%
  mutate(N1RankLeft = rank(N1- lag(N1)),
         N1RankRight = rank(N1 - lead(N1)),
         N2RankLeft = rank(N2- lag(N2)),
         N2RankRight = rank(N2 - lead(N2)))%>%
  mutate(
          MaxRank = pmax(N1RankLeft,N1RankRight,N2RankLeft,N2RankRight),
  ...
  select(Date,Site,N1,N2,MaxRank,ChangeLeadN1,DiffLeadN1,RatLeadN1, ChangeMinN1)%>%
  arrange(Site, Date)
...
#MaxRank
PlotlyView(DiffGen, "N1", "MaxRank", QuintileBound, Threshold = BreakUsed)
...
RankingDF <- LIMSFullDF%>%
  ...
  mutate(N1RankLeft = rank(desc(N1- lag(N1))),
         N1RankRight = rank(desc(lead(N1) - N1)),
         N2RankLeft = rank(desc(N2- lag(N2))),
         N2RankRight = rank(desc(lead(N2)-N2)))%>%
  select(Date,Site,N1RankLeft,N1RankRight,N2RankLeft,N2RankRight)%>%
  arrange(N1RankLeft)
RankingDF
write.csv(RankingDF,"RmdOutput/RankingDF.csv",quote = FALSE)

RankingDF%>%
  mutate(DiffN1 = N1RankLeft - N1RankRight)%>%
  arrange(DiffN1)
#
P
BreakUsed <- c(0)
PlotlyView(DiffGen, "N1", "N1", QuintileBound, Threshold = BreakUsed)
PlotlyView(DiffGen, "N2", "N2", QuintileBound, Threshold = BreakUsed)
#Norm Sqrt?,


RankingDF%>%
  mutate(DiffN1 = pmax(N1RankLeft,N1RankRight,N2RankLeft,N2RankRight))%>%
  arrange(DiffN1)

DiffPlot <- LIMSFullDF%>%
  ggplot(aes(x = Date, y = N1 - lag(N1)))+
  geom_point()+
  facet_wrap(~Site)
