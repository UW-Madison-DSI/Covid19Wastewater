...
output:
  ...
  html_document:
    df_print: paged
params:
  BaseDir: ~/Box/DSI/Projects/wastewater/RD/
editor_options:
...
ggarrange(BaseOutlierPlot, ZoomedOutlierPlot, nrow = 2, align ="v", legend.grob=get_legend(BaseOutlierPlot), legend="right")
```
...
IntercepterOverLay <- IntercepterDF%>%
  ...
  scale_y_log10()+
  ggtitle("Interceptors Concentration")
...
ggarrange(IntercepterChangeOverLay, MadChangeOverLay, 
          nrow = 2, align = "v", common.legend = TRUE, legend = "right")
Lim <- 1e6
ZoomSettings <- scale_y_continuous(limits = c(-Lim,Lim))
ggarrange(IntercepterChangeOverLay+ZoomSettings, 
          MadChangeOverLay+ZoomSettings, common.legend = TRUE,
          nrow = 2, align = "v", legend = "right")
