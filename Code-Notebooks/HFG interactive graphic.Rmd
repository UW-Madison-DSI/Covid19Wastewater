...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            checkboxInput(inputId = "Line", label = ("Daily mean"),
                          value=F
            ),
            checkboxInput(inputId = "Means", label = ("mean of qPCR"),
                          value=F
            ),
        ...
        #creates panel
        mainPanel(div("Info About Data"),plotOutput("plot2",inline=TRUE), br(),div("Discription Of Data"))
    )
)
```
...
server <- function(input, output) {
    ...
    output$plot2<-renderPlot(
        ...
        height = function() 200+350*ifelse(input$Plant=="All",4,1),
        {
            ...
            if(input$Means==T){
                set.seed(myseed)
                HFGDPlot=HFGDPlot+geom_jitter(shape=23,aes(y=Mean.N1GC,fill=Filter),size=5,height=0,width=.1,data=Filtered_mean())
                #if(input$EB=="On"){
