...
server <- function(input, output) {
  ...
  Filtered_mean = reactive({
    filtered_data()%>%
      group_by(Date,Filter,Plant)%>%
      summarize(Mean.N1GC=mean(N1GC))
  })
  
  output$plot2<-renderPlot({
      HFGDPlot=ggplot()+aes(x=Date,color=Filter)+
        geom_jitter(data=filtered_data(),aes(y=N1GC,shape=Plant,fill=Filter),height=0,width=.1,size=3)
