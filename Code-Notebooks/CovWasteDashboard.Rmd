...
DCovDF=CovidDataDorms(Spring,Fall)%>%
  ...
  mutate(rollingPer_pos=roll_mean(Per_pos,n=7,fill=NA))
...
ui <- dashboardPage(
  ...
  dashboardSidebar(sidebarMenu(
    menuItem("UW Dorms waste water", tabName = "MadDash", icon = icon("dashboard")),
...
server <- function(input, output) {
  ...
  Make_Plots1 = reactive({
    ...
    if("Percent of tests positive" %in% input$Vars){
      if(input$'7day'){
        DormPlot=ggplot(data=filtered_data_PCov(),aes(y=rollingPer_pos,x=Date))
      } else{
        DormPlot=ggplot(data=filtered_data_PCov(),aes(y=Per_pos,x=Date))
      }
      DormPlot=DormPlot+
        geom_jitter(alpha=.7,height=0,width=.1,size=2)+ylab("Percent Positive")+
        facet_wrap(~Site, nrow = 1)+coord_cartesian(xlim=c(minC(), maxC()))+
        theme(text = element_text(size=20),axis.text.y = element_text(size = 15, colour = "black")) + theme(strip.text.x = element_text(size = 20, colour = "black"))+ rremove("xlab")
  ...
  output$plot1<-renderPlot(
    ...
    height = function() 60+300*length(input$Vars),
    {
      ...
      return(annotate_figure(top=text_grob("UW dorms Covid Visualization",size=20),ggarrange(plotlist=plots,ncol =1,align="v")))
    })
  #-------------------------
  #required for tab2
